...
let
  ...
  changeWallpaperScript = pkgs.writeShellScriptBin "change-wallpaper" ''
    ...
    if [ -n "$WALLPAPER" ]; then
      awww img "$WALLPAPER" --transition-fps 60 --transition-duration 2 --transition-type any --transition-pos top-right --transition-bezier .3,0,0,.99 --transition-angle 135
    fi
  '';
in
{
