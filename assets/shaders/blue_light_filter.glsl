/*
 * Blue Light Filter (template: __STRENGTH__ and __TEMPERATURE__ replaced at runtime)
 * Source: https://github.com/loqusion/hyprshade (blue-light-filter.glsl.mustache)
 */
...
vec3 colorTemperatureToRGB(const in float temperature) {
  ...
  return mix(clamp(m[0] / (vec3(clamp(temperature, 1000.0, 40000.0)) + m[1]) + m[2], 0.0, 1.0), vec3(1.0), smoothstep(1000.0, 0.0, temperature));
}
